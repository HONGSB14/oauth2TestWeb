    ...
    public class SecurityConfig extends WebSecurityConfigurerAdapter {                    //WebSecurityConfigurerAdapter 상속받은 클래스이다.
        ...
        protected void configure(HttpSecurity http) throws Exception {                               //HTTP(URL) 관련 시큐리티 보안구성 메소드
            http.authorizeHttpRequests().antMatchers("/").permitAll()            // 모든 페이지의 권한을 열어준다.
                    ...
                    .loginProcessingUrl("/member/loginController")                                           //로그인 유효성 검사를 할 URL를 지정한다.
        ...
        @Override   //재정의 한다.
        protected void configure( AuthenticationManagerBuilder auth ) throws Exception{  //Security기본값 설정 대신 사용자가원하는 커스텀으로 만들기 위한 메소드 정의
            auth.userDetailsService(indexService).passwordEncoder(new BCryptPasswordEncoder());  //패스워드는 BCryptPassword 로하는 USER 커스텀 정의한다.
        }
}
